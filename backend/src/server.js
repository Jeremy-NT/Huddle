...
//it is important that you don't pass the clerk webhook data, it should be in raw format
// it's important that you dont pass the clerk webhook data through express.json() middleware, as it will parse the request body and modify the data. 
// Instead, you should use express.raw() middleware to handle the raw request body and pass it to the clerkWebhook function for processing.
app.use("/api/webhooks/clerk",express.raw({type: 'application/json'}),clerkWebhook)// Use the Clerk webhook middleware to handle incoming webhook requests from Clerk
